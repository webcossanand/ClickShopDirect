(function ($) {
    ...
    var PAPROWidgetsEditor = {
        ...
        init: function () {
            ...
            PAPROWidgetsEditor.activatePathGenerator();

            window.elementor.on('preview:loaded', function () {

                elementor.$preview[0].contentWindow.PAPROWidgetsEditor = PAPROWidgetsEditor;

            });

            elementor.channels.editor.on('section:activated', PAPROWidgetsEditor.onSectionActivate);

            elementor.on('navigator:init', PAPROWidgetsEditor.onNavigatorInit);

            elementor.channels.editor.on('refreshElement', function (e) {
                $e.run('document/elements/settings', { container: e.container, settings: {}, options: { external: true } });
            });
        ...
        onSectionActivate: function (sectionName, elementorEditor) {
            ...
            window.PAPROWidgetsEditor.currentEditModel = editModelView;
        ...
        activatePathGenerator: function () {
            elementor.channels.editor.on("generate", function (e) {
                ...
                var data = e._parent.model,
                    ...
                    blobHtml = PAPROWidgetsEditor.drawBlob(blobsAttr, newPath),
                    params = {
                        'seed': seed,
                        'html': blobHtml,
                        'path': newPath
            ...
            if (blobAttr) {
                ...
                $.each(blobs, function (index, blob) {
                    ...
                    if ('pre' === blob.type) {
                        ...
                        blobHtml = PAPROWidgetsEditor.drawBlob(blob, path);
        ...
        getBlobModels: function ($scope) {
            ...
            var $parentContainer = $scope.closest(".e-con[data-nesting-level=0]"),
                isContainer = true;

            //If Flex Container is disabled, or it's enabled, but the current parent is still a section.
            if (!elementorFrontend.config.experimentalFeatures.container || $parentContainer.length < 1) {
                isContainer = false;
                $parentContainer = $scope.closest(".elementor-top-section");
            }
            ...
            $.each(editorElements.models, function (index, elem) {
                if (targetId == elem.id) {
                ...
                } else if (elem.id === $parentContainer.data("id")) {
                    ...
                    if (!isContainer) {
                        ...
                        $.each(elem.attributes.elements.models, function (index, col) {
                            ...
                            if (targetId == col.id) { // found the container
        ...
        drawBlob: function (blob, path) {
            ...
            var blobHtml = '<svg width="' + blob.size + '" height="' + blob.size + '" viewBox="0 0 ' + blob.size + ' ' + blob.size + '" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false">';
        ...
        handlePageSettingsTrigger: function () {

            $(document).on('click', '.premium-page-setting-trigger', function (e) {
                e.preventDefault();

                var $pageSettingsButton = $('#elementor-panel-footer-settings');

                if ($pageSettingsButton.length) {
                    $pageSettingsButton.trigger('click');

                    var targetSection = $('.elementor-control-section_premium_cursor');
    ...
    $(window).on('elementor:init', PAPROWidgetsEditor.init);

    window.PAPROWidgetsEditor = PAPROWidgetsEditor;
