...
use PremiumAddonsPro\Includes\White_Label;
...
class Admin_Helper {
	...
	public function __construct() {
		...
		add_action( 'plugins_loaded', array( $this, 'handle_license_check' ) );

		add_action( 'after_setup_theme', array( $this, 'init_white_labeling_branding' ), 9 );
	}
	...
	public function add_admin_tab( $tabs ) {
		...
		$white_label = White_Label\Helper::get_white_labeling_settings();
	...
	public function handle_license_check() {
		...
		$did_check = get_transient( 'pa_license_info' );
		...
		if ( ! $did_check ) {
			...
			// When the request is not going to our server, we need to prevent calling it everytime.
			if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
				set_transient( 'pa_license_info', true, WEEK_IN_SECONDS );
				return;
			}
			...
			if ( isset( $body['license'] ) ) {
				...
				// TODO: replace `pa_license_check` transient.
				// set_transient( 'pa_license_check', $body['license'], WEEK_IN_SECONDS );
				...
				set_transient( 'pa_license_info', $data, WEEK_IN_SECONDS );

				if ( 'expired' === $body['license'] ) {
					API::papro_deactivate_license( $license, $body['license'] );
				}
			}
		}
	}
	...
	public function init_white_labeling_branding() {
