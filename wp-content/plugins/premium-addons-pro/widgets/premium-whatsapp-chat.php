...
class Premium_Whatsapp_Chat extends Widget_Base {
	...
	public function get_keywords() {
		return array( 'pa', 'premium', 'premium whatsapp chat', 'message', 'client', 'send', 'customer' );
	}
    ...
    public function get_script_depends() {
		...
		if ( $is_edit ) {
			...
			$scripts = array_merge( $draw_scripts, array( 'lottie-js', 'tooltipster-bundle' ) );

		} else {
			$settings = $this->get_settings();

            if ( 'yes' === $settings['tooltips'] ) {
				$scripts[] = 'tooltipster-bundle';
			}
