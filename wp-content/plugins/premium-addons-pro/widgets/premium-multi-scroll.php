...
class Premium_Multi_Scroll extends Widget_Base {
	...
	public function get_keywords() {
		return array( 'pa', 'premium', 'premium multi scroll', 'animation', 'split', 'half', 'slider' );
	}
	...
	protected function get_repeater_controls( $repeater, $condition = array() ) {
		...
		$repeater->add_control(
			...
			array(
				...
				'separator'   => 'before',
				'label_block' => true,
				'description' => __( 'Use this to add an element selector to be used to navigate to this slide. Check this <a href="https://premiumaddons.com/docs/how-to-use-elementor-elements-to-navigate-through-multi-scroll-slides/" target="_blank">article</a>.', 'premium-addons-for-elementor' ),
			)
		);
		...
		$repeater->add_control(
			'fixed_content',
			array(
				'label'     => __( 'Fixed Content', 'elementor' ),
	...
	protected function register_controls() { // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
		...
		$this->add_control(
			'left_side_repeater',
			array(
				'label'  => __( 'Containers', 'premium-addons-pro' ),
		...
		$this->add_control(
			...
			array(
				...
				'type'         => Controls_Manager::SWITCHER,
				'prefix_class' => 'premium-multiscroll-cposition-',
			)
		);
		...
		$this->add_responsive_control(
			...
			array(
				...
				'selectors'  => array(
					'{{WRAPPER}} .premium-multiscroll-fixed' => 'left: {{SIZE}}{{UNIT}}',
				),
				'condition'  => array(
					'fixed_custom_offset' => 'yes',
				),
			)
		);
		...
		$this->add_responsive_control(
			...
			array(
				...
				'selectors'  => array(
					'{{WRAPPER}} .premium-multiscroll-fixed' => 'top: {{SIZE}}{{UNIT}}',
				),
				'condition'  => array(
					'fixed_custom_offset' => 'yes',
				),
		...
		$this->add_responsive_control(
			...
			array(
				...
				'selectors'  => array(
					'{{WRAPPER}} .premium-multiscroll-fixed' => 'width: {{SIZE}}{{UNIT}}',
				),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'default'   => 1,
				'selectors' => array(
					'{{WRAPPER}} .premium-multiscroll-fixed' => 'z-index: {{VALUE}}',
				),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'type'      => Controls_Manager::SELECT,
				'options'   => array(
					'circ'  => __( 'Circles', 'premium-addons-pro' ),
					'lines' => __( 'Lines', 'premium-addons-pro' ),
				),
				'default'   => 'circ',
				'condition' => array(
					'navigation_dots' => 'yes',
		...
		$this->add_control(
			'show_on_tooltips',
			array(
				'label'     => __( 'Show On Tooltips', 'premium-addons-pro' ),
				'type'      => Controls_Manager::SELECT,
				'options'   => array(
		...
		$this->add_control(
			...
			array(
				...
				'options'   => array(
					...
					'h-middle' => __( 'Middle', 'premium-addons-pro' ),
		...
		$this->add_control(
			'tooltips_position',
			array(
				'label'     => __( 'Tooltips Position', 'premium-addons-pro' ),
				'type'      => Controls_Manager::SELECT,
				'options'   => array(
					'h-right'     => __( 'Right', 'premium-addons-pro' ),
					'h-left'      => __( 'Left', 'premium-addons-pro' ),
					'alternative' => __( 'Alternative', 'premium-addons-pro' ),
				),
				'default'   => 'alternative',
				'condition' => array(
					'navigation_dots'     => 'yes',
					'navigation_dots_pos' => 'h-middle',
				),
			),
		);
		...
		$this->add_control(
			...
			array(
				...
				'selectors' => array(
					...
					'{{WRAPPER}} .multiscroll-nav li.lines a.active span::before' => 'color: {{VALUE}}',
				),
			)
		);
		...
		$this->start_controls_section(
			'fixed_content_style',
			array(
				'label' => __( 'Fixed Content', 'premium-addons-pro' ),
				'tab'   => CONTROLS_MANAGER::TAB_STYLE,
			)
		);
		...
		$this->add_group_control(
			...
			array(
				'name'     => 'fixed_content_background',
				'types'    => array( 'classic', 'gradient' ),
				'selector' => '{{WRAPPER}} .premium-multiscroll-fixed',
			)
		);
		...
		$this->add_group_control(
			Group_Control_Border::get_type(),
			array(
				'name'     => 'fixed_content_border',
				'selector' => '{{WRAPPER}} .premium-multiscroll-fixed',
			)
		);

		$this->add_control(
			'fixed_content_border_radius',
			array(
				'label'      => __( 'Border Radius', 'premium-addons-pro' ),
				'type'       => Controls_Manager::SLIDER,
				'size_units' => array( 'px', '%', 'em' ),
				'selectors'  => array(
					'{{WRAPPER}} .premium-multiscroll-fixed' => 'border-radius: {{SIZE}}{{UNIT}};',
				),
			)
		);

		$this->add_responsive_control(
			'fixed_content_margin',
			array(
				'label'      => __( 'Margin', 'premium-addons-pro' ),
				'type'       => Controls_Manager::DIMENSIONS,
				'size_units' => array( 'px', 'em', '%' ),
				'selectors'  => array(
					'{{WRAPPER}} .premium-multiscroll-fixed' => 'margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				),
			)
		);

		$this->add_responsive_control(
			'fixed_content_padding',
			array(
				'label'      => __( 'Padding', 'premium-addons-pro' ),
				'type'       => Controls_Manager::DIMENSIONS,
				'size_units' => array( 'px', 'em', '%' ),
				'selectors'  => array(
					'{{WRAPPER}} .premium-multiscroll-fixed' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				),
			)
		);
	...
	protected function render() {
		...
		$scoll_settings = array(
			...
			'tooltipsTrigger'   => $settings['show_on_tooltips'],
			'tooltipsPosition'  => $settings['tooltips_position'],
			'topLoop'           => $top_loop,
			'btmLoop'           => $bottom_loop,
			'anchors'           => $anchors_arr,
		...
		$this->add_render_attribute(
			'multiscroll_wrapper',
			array(
				'class'         => 'premium-multiscroll-wrap',
				'data-settings' => wp_json_encode( $scoll_settings ),
		...
		<div <?php echo wp_kses_post( $this->get_render_attribute_string( 'multiscroll_wrapper' ) ); ?>>
			...
			<div <?php echo wp_kses_post( $this->get_render_attribute_string( 'multiscroll_inner' ) ); ?>>
				...
				<div class="premium-multiscroll-fixed">
					...
					foreach ( $templates as $index => $section ) :
						...
						$this->add_render_attribute(
							'fixed_content_template' . $index,
							'class',
							array(
								'premium-multiscroll-fixed-temp',
								'premium-addons__v-hidden',
							)
						);

						?>
						<?php if ( ! empty( $section['fixed_content_template'] ) ) : ?>
							<div <?php echo wp_kses_post( $this->get_render_attribute_string( 'fixed_content_template' . $index ) ); ?>  data-section-index="<?php echo esc_attr( $index ); ?>">
								<?php
									$template = empty( $section['fixed_content_template'] ) ? $section['live_temp_fixed_content'] : $section['fixed_content_template'];
									echo Helper_Functions::render_elementor_template( $template ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
								?>
							</div>
						<?php endif; ?>
