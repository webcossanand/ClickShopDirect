...
class Premium_Unfold extends Widget_Base {
	...
	public function get_keywords() {
		return array( 'pa', 'premium', 'premium unfold', 'read', 'section', 'more', 'cta', 'content' );
	}
	...
	protected function register_controls() { // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
		...
		$this->add_control(
			...
			array(
				...
				'size_units' => array( 'px', 'em', '%' ),
				'selectors'  => array(
					'{{WRAPPER}} .premium-button' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}}',
				),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'size_units' => array( 'px', '%', 'em' ),
				'selectors'  => array(
					'{{WRAPPER}} .premium-button:hover' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}}',
				),
			)
		);
			...
			<span class="premium-icon-holder-fold">
				...
				if ( $fold_migrated || $fold_is_new ) :
					Icons_Manager::render_icon( $settings['premium_unfold_button_icon_updated'], array( 'aria-hidden' => 'true' ) );
						...
						?>
					<i class ="<?php echo esc_attr( $settings['premium_unfold_button_icon'] ); ?>" aria-hidden="true" focusable="false"></i>
				<?php endif; ?>
			</span>
			<span class="premium-icon-holder-unfolded">
				...
				if ( $unfold_migrated || $unfold_is_new ) :
					Icons_Manager::render_icon( $settings['premium_unfold_button_icon_unfolded_updated'], array( 'aria-hidden' => 'true' ) );
						...
						?>
					<i class ="<?php echo esc_attr( $settings['premium_unfold_button_icon_unfolded'] ); ?>" aria-hidden="true" focusable="false"></i>
				<?php endif; ?>
			</span>
		<?php endif; ?>
		<?php
