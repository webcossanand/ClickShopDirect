...
class Premium_Divider extends Widget_Base {
	...
	public function get_keywords() {
		return array( 'pa', 'premium', 'premium divider', 'lottie', 'separator', 'svg', 'icon' );
	}
	...
	public function get_script_depends() {
		...
		if ( $is_edit ) {
			...
			if ( 'yes' === $settings['draw_svg'] || 'yes' === $settings['sep_draw_svg'] ) {
	...
	private function add_divider_controls( $draw_icon ) {
        ...
        if ( version_compare( PREMIUM_ADDONS_VERSION, '4.11.24', '>' ) ) {
            $demo = Helper_Functions::get_campaign_link( 'https://premiumaddons.com/divider-widget-for-elementor-page-builder/', 'divider', 'wp-editor', 'demo' );
            Helper_Functions::add_templates_controls( $this, 'divider', $demo );
        }
	...
	protected function render() {
		...
		if ( $left_sep || $right_sep ) {
			...
			if ( $bg_sep ) {
				...
				} elseif ( ! in_array( $separator, array( 'div-bg-diamond', 'div-bg-para', 'div-bg-rect' ), true ) ) {
