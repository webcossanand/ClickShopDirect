...
class Premium_Yelp_Reviews extends Widget_Base {
	...
	public function get_keywords() {
		return array( 'pa', 'premium', 'premium yelp reviews', 'business', 'rating', 'testimonials', 'place', 'rate', 'recommendation', 'social' );
	}
	...
	protected function register_controls() { // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
		...
		foreach ( $docs as $url => $title ) {
			...
			$doc_url = Helper_Functions::get_campaign_link( $url, 'yelp-widget', 'wp-editor', 'get-support' );
	...
	protected function render() {
		...
		if ( false === $response || false === $place_response ) {
			...
			if ( is_wp_error( $response_reviews ) || 200 !== wp_remote_retrieve_response_code( $response_reviews ) ) {
				?>
				<div class="premium-error-notice">
					<?php echo wp_kses_post( 'Something went wrong' ); ?>
				</div>
				<?php
				return;
			}
		...
		if ( ! $response || ! property_exists( $response, 'reviews' ) ) {
			?>
			<div class="premium-error-notice">
