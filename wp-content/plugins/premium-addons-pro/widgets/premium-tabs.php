...
class Premium_Tabs extends Widget_Base {
    ...
    public function get_script_depends() {
		...
		if ( $is_edit ) {
			...
			$scripts = array_merge( $draw_scripts, array( 'pa-glass', 'pa-slick', 'lottie-js' ) );

		} else {
			$settings = $this->get_settings();

			if ( ! empty( $settings['premium_tabs_repeater'] ) ) {

                $draw_js = false;
                $lottie_js = false;

				foreach ( $settings['premium_tabs_repeater'] as $item ) {
					...
					if ( ! $lottie_js && 'animation' === $item['icon_type'] ) {
						$scripts[] = 'lottie-js';
						$lottie_js = true;
					}
			...
			if ( 'true' === $settings['carousel_tabs'] ) {
				$scripts[] = 'pa-slick';
			}

            if ( 'none' !== $settings['tabs_lq_effect'] || 'none' !== $settings['content_lq_effect'] ) {
				$scripts[] = 'pa-glass';
			}
		}
	...
	public function get_keywords() {
		return array( 'pa', 'premium', 'premium tabs', 'content', 'switcher', 'section' );
	}
	...
	protected function register_controls() { // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
        ...
        if ( version_compare( PREMIUM_ADDONS_VERSION, '4.11.24', '>' ) ) {
            $demo = Helper_Functions::get_campaign_link( 'https://premiumaddons.com/elementor-tabs-widget/', 'tabs', 'wp-editor', 'demo' );
            Helper_Functions::add_templates_controls( $this, 'tabs', $demo );
        }
        ...
        $this->add_control(
			'moving_background',
			array(
				'label'              => __( 'Moving Background', 'premium-addons-pro' ),
				'type'               => Controls_Manager::SWITCHER,
                'prefix_class' => 'premium-tabs__background-',
                'condition'          => array(
					'premium_tab_style_selected' => 'style1',
				),
                'frontend_available' => true,
		...
		$this->add_control(
			'accordion_notice',
			array(
				'raw'             => __( 'To see the accordion, you will need to check the page on a real tablet/mobile device.', 'premium-addons-pro' ),
				'type'            => Controls_Manager::RAW_HTML,
				'content_classes' => 'elementor-panel-alert elementor-panel-alert-info',
				'condition'       => array(
					'carousel_tabs!' => 'true',
		...
		foreach ( $docs as $url => $title ) {
			...
			$doc_url = Helper_Functions::get_campaign_link( $url, 'tabs-widget', 'wp-editor', 'get-support' );
		...
		$this->add_group_control(
			...
			array(
				...
				'fields_options' => array(
					'border' => array(
						'selectors' => array(
							'{{WRAPPER}}:not(.premium-tabs__background-yes) .premium-tabs .premium-tabs-nav-list li.tab-current .premium-tab-link, {{WRAPPER}}.premium-tabs__background-yes .premium-tabs-nav-list::before' => 'border-style: {{VALUE}}',
						),
					),
					'width'  => array(
						'selectors' => array(
							'{{WRAPPER}}:not(.premium-tabs__background-yes) .premium-tabs .premium-tabs-nav-list li.tab-current .premium-tab-link' => 'border-width: {{TOP}}px {{RIGHT}}px {{BOTTOM}}px {{LEFT}}px',
                            '{{WRAPPER}}.premium-tabs__background-yes .premium-tabs-nav-list::before' => 'border-width: {{TOP}}px {{RIGHT}}px {{BOTTOM}}px {{LEFT}}px;',
					...
					'color'  => array(
						'selectors' => array(
							'{{WRAPPER}}:not(.premium-tabs__background-yes) .premium-tabs .premium-tabs-nav-list li.tab-current .premium-tab-link' => 'border-color: {{VALUE}}',
                            '{{WRAPPER}}.premium-tabs__background-yes .premium-tabs-nav-list::before' => 'border-color: {{VALUE}};',
		...
		$this->add_control(
			...
			array(
				...
				'size_units' => array( 'px', '%', 'em' ),
				'selectors'  => array(
					'{{WRAPPER}}:not(.premium-tabs__background-yes) .premium-tabs .premium-tabs-nav-list li.tab-current .premium-tab-link, {{WRAPPER}}.premium-tabs__background-yes .premium-tabs-nav-list::before' => 'border-radius: {{SIZE}}{{UNIT}};',
				),
				'condition'  => array(
					'premium_tab_style_selected' => array( 'style1', 'style3' ),
				),
		...
		$this->add_group_control(
			...
			array(
				...
				'selector'  => '{{WRAPPER}}:not(.premium-tabs__background-yes) .premium-tabs:not(.premium-tabs-style-circle) .tab-current .premium-tab-link, {{WRAPPER}}.premium-tabs__background-yes .premium-tabs-nav-list::before, {{WRAPPER}} .premium-tabs-style-circle .premium-tabs-nav li.tab-current::before',
				'condition' => array(
					'premium_tab_style_selected!' => 'style4',
				),
			)
        ...
        $this->start_controls_section(
			'tabs_container_style_section',
			array(
				'label' => __( 'Tabs Container', 'premium-addons-pro' ),
				'tab'   => Controls_Manager::TAB_STYLE,
			)
		);
        ...
        $this->add_responsive_control(
			...
			array(
				...
				'type'      => Controls_Manager::SLIDER,
				'selectors' => array(
					'{{WRAPPER}} .premium-tabs-nav' => 'width: {{SIZE}}%',
				),
			)
		);

        $this->add_control(
			'tabs_container_alignment',
			array(
                ...
                'prefix_class' => 'premium-tabs__align-',
				'condition' => array(
					'tabs_container_width!' => '',
				),
			)
        ...
        $this->add_control(
			...
			array(
				...
				'type'      => Controls_Manager::COLOR,
				'selectors' => array(
					'{{WRAPPER}} .premium-tabs-nav' => 'background-color: {{VALUE}};',
				),
			)
		);
        ...
        $this->add_group_control(
			...
			array(
				...
				'selector'       => '{{WRAPPER}} .premium-tabs-nav',
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'size_units' => array( 'px', '%', 'em' ),
				'selectors'  => array(
					'{{WRAPPER}} .premium-tabs-nav' => 'border-radius: {{SIZE}}{{UNIT}};',
				),
			)
		);
        ...
        $this->add_responsive_control(
			...
			array(
				...
				'size_units' => array( 'px', 'em', '%' ),
				'selectors'  => array(
					'{{WRAPPER}} .premium-tabs-nav' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}}',
				),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'selectors' => array(
					...
					'{{WRAPPER}} .premium-tabs-nav .premium-svg-nodraw > svg, {{WRAPPER}} .premium-tabs-nav .premium-svg-nodraw > svg *' => 'fill: {{VALUE}};',
				),
			)
		);
			...
			$this->add_control(
				...
				array(
					...
					'selectors' => array(
						...
						'{{WRAPPER}} .premium-tabs-nav .premium-svg-nodraw > svg *' => 'stroke: {{VALUE}};',
					),
				)
			);
		}
		...
		$this->add_control(
			...
			array(
				...
				'selectors' => array(
					...
					'{{WRAPPER}} .premium-tabs-nav-list-item:hover .premium-tab-link > svg, {{WRAPPER}} .premium-tabs-nav-list-item:hover .premium-tab-link > svg *' => 'fill: {{VALUE}};',
					'{{WRAPPER}} .premium-tabs-nav-list-item:hover .premium-svg-nodraw > svg, {{WRAPPER}} .premium-tabs-nav-list-item:hover .premium-svg-nodraw > svg *' => 'fill: {{VALUE}};',
		...
		if ( $draw_icon ) {
			$this->add_control(
				...
				array(
					...
					'selectors' => array(
						'{{WRAPPER}} .premium-tabs-nav-list-item:hover .premium-tab-link > svg *' => 'stroke: {{VALUE}};',
						'{{WRAPPER}} .premium-tabs-nav-list-item:hover .premium-svg-nodraw > svg *' => 'stroke: {{VALUE}};',
					),
				)
			);
		}
		...
		$this->add_control(
			...
			array(
				...
				'selectors' => array(
					...
					'{{WRAPPER}} .premium-tabs-nav .tab-current .premium-svg-nodraw > svg, {{WRAPPER}} .premium-tabs-nav .tab-current .premium-svg-nodraw > svg *' => 'fill: {{VALUE}}',
			...
			$this->add_control(
				...
				array(
					...
					'selectors' => array(
						...
						'{{WRAPPER}} .premium-tabs-nav .tab-current .premium-svg-nodraw > svg *' => 'stroke: {{VALUE}};',
					),
				)
			);
		}
						...
						foreach ( $tabs as $index => $tab ) {
							...
							$this->add_render_attribute(
								...
								array(
									'class' => array(
										'premium-tab-link',
										'premium-con-lq__' . $settings['tabs_lq_effect'],
									),
									'href'  => '#section-' . $section_style . '-' . $index . '-' . $this->get_id(),
								)
							);

							?>
							<li <?php echo wp_kses_post( $this->get_render_attribute_string( $list_key ) ); ?>>
								<a <?php echo wp_kses_post( $this->get_render_attribute_string( $tab_key ) ); ?>>
									...
									if ( 'yes' === $tab['premium_tabs_icon_switcher'] ) {
										if ( 'icon' === $tab['icon_type'] ) {
											...
											echo Helper_Functions::get_svg_by_icon(
												...
												$this->get_render_attribute_string( $icon_key )
											);
								...
								<?php if ( 'style1' === $style && 'yes' === $settings['show_arrow'] ) : ?>
								...
								?>
									<li class="premium-accordion-tab-content" id="premium-accordion-content-<?php echo esc_attr( $index ); ?>">
										<section id="section-<?php echo esc_attr( $section_style . '-' . $index . '-' . $this->get_id() ); ?>" class="premium-tabs-content-section">
											<div class="premium-tab-content <?php echo esc_attr( 'premium-con-lq__' . $settings['content_lq_effect'] ); ?>">
