...
class Premium_Img_Layers extends Widget_Base {
	...
	public function get_keywords() {
		return array( 'pa', 'premium', 'premium image layers', 'float', 'parallax', 'mouse', 'interactive', 'advanced' );
	}
    ...
    public function get_script_depends() {
		...
		if ( $is_edit ) {
			...
			$scripts = array_merge( $draw_scripts, array( 'pa-scroll-effects', 'pa-tilt', 'pa-anime', 'lottie-js' ) );

		} else {
			$settings = $this->get_settings();

			if ( ! empty( $settings['premium_img_layers_images_repeater'] ) ) {

                $draw_js = false;
                $lottie_js = false;

				foreach ( $settings['premium_img_layers_images_repeater'] as $item ) {
					...
					if ( ! $lottie_js && 'animation' === $item['media_type'] ) {
						$scripts[] = 'lottie-js';
						$lottie_js = true;
					}

                    if ( 'yes' === $item['premium_img_layers_scroll_effects'] ) {
						$scripts[] = 'pa-scroll-effects';
					}

                    if ( 'yes' === $item['premium_img_layers_float_effects'] ) {
						$scripts[] = 'pa-anime';
					}

                    if( 'yes' === $item['premium_img_layers_mouse'] ) {
                        if ( 'parallax' === $item['premium_img_layers_mouse_type'] ) {
                            $scripts[] = 'pa-tweenmax';
                        } else {
                            $scripts[] = 'pa-tilt';
                        }
	...
	protected function register_controls() { // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
		...
		$this->add_control(
			'refresh_button',
			array(
				'label'       => __( 'Click This Button on Window Resize', 'premium-addons-pro' ),
				'type'        => Controls_Manager::BUTTON,
				'button_type' => 'default',
				'text'        => __( 'Refresh', 'premium-addons-pro' ),
				'event'       => 'refreshElement',
			)
		);
		...
		if ( $draw_icon ) {
			$repeater->add_control(
				'svg_notice',
				array(
					'raw'             => __( 'Loop and Speed options are overridden when Draw SVGs in Sequence option is enabled.', 'premium-addons-pro' ),
		...
		$this->add_responsive_control(
			...
			array(
				...
				'selectors'  => array(
					'{{WRAPPER}} .premium-img-layers-list-wrapper' => 'min-height: {{SIZE}}{{UNIT}}',
				),
			)
		);
		...
		$this->add_responsive_control(
			...
			array(
				...
				'default'   => 'visible',
				'selectors' => array(
					'{{WRAPPER}} .premium-img-layers-list-wrapper'   => 'overflow: {{VALUE}}',
				),
			)
		);
		...
		foreach ( $docs as $url => $title ) {
			...
			$doc_url = Helper_Functions::get_campaign_link( $url, 'layers-widget', 'wp-editor', 'get-support' );
		...
		<div <?php echo wp_kses_post( $this->get_render_attribute_string( 'container' ) ); ?>>
			<ul class="premium-img-layers-list-wrapper">
				...
				foreach ( $settings['premium_img_layers_images_repeater'] as $index => $image ) :
					...
					if ( 'yes' === $image['premium_img_layers_animation_switcher'] && ! empty( $image['premium_img_layers_animation'] ) ) {
						...
						$animation_settings = array(
							...
							'delay'     => $image['premium_img_layers_animation_delay'],
						);

						array_push( $e_animations, 'e-animation-' . $animation_class );

						$this->add_render_attribute( $list_item_key, 'style', 'opacity: 0' );
						$this->add_render_attribute( $list_item_key, 'data-settings', wp_json_encode( $animation_settings ) );
					}
					...
					?>
						...
						if ( 'image' === $image['media_type'] ) {
							...
							$text_tag = PAPRO_Helper::validate_html_tag( $image['img_layer_text_tag'] );

							$text_class = 'premium-img-layers-text ' . $dir_class;

							?>
								<<?php echo wp_kses_post( $text_tag ); ?> class="<?php echo esc_attr( $text_class ); ?>">
							...
							if ( 'svg' === $image['svg_icon'] ) {
								...
								echo Helper_Functions::get_svg_by_icon( $image['font_icon'] );
							}
						}
						?>
				...
				if ( ! \Elementor\Plugin::$instance->editor->is_edit_mode() ) {
					PAPRO_Helper::load_e_animation_assets( 'styles', $e_animations ); // load entrance animation assets.
				}
				?>
			</ul>
		</div>
