...
class Premium_Flipbox extends Widget_Base {
    ...
    public function get_script_depends() {
		...
		if ( $is_edit ) {
			...
			if ( 'yes' === $settings['front_draw_svg'] || 'yes' === $settings['back_draw_svg'] ) {
				array_push( $scripts, 'pa-tweenmax', 'pa-motionpath' );
			}

			if ( 'animation' === $settings['premium_flip_icon_selection'] || 'animation' === $settings['premium_flip_back_icon_selection'] ) {
				$scripts[] = 'lottie-js';
			}

            if ( 'none' !== $settings['front_lq_effect'] || 'none' !== $settings['back_lq_effect'] ) {
				$scripts[] = 'pa-glass';
			}
		}
	...
	public function get_keywords() {
		return array( 'pa', 'premium', 'premium 3d hover box', 'flip box', '3d', 'rotate', 'fade', 'info', 'animation' );
	}
	...
	protected function register_controls() { // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
        ...
        if ( version_compare( PREMIUM_ADDONS_VERSION, '4.11.24', '>' ) ) {
            $demo = Helper_Functions::get_campaign_link( 'https://premiumaddons.com/3d-hover-box-flip-box-widget-for-elementor/', 'hover-box', 'wp-editor', 'demo' );
            Helper_Functions::add_templates_controls( $this, 'hover-box', $demo );
        }
		...
		$doc1_url = Helper_Functions::get_campaign_link( 'https://premiumaddons.com/docs/flip-box-widget-tutorial/', 'flipbox-widget', 'wp-editor', 'get-support' );
		...
		$this->add_responsive_control(
			...
			array(
				...
				'size_units' => array( 'px', 'em', '%' ),
				'selectors'  => array(
					'{{WRAPPER}} .premium-flip-front-content-container' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}}  {{LEFT}}{{UNIT}};',
				),
			)
		);
		...
		$this->add_responsive_control(
			...
			array(
				...
				'size_units' => array( 'px', 'em', '%' ),
				'selectors'  => array(
					'{{WRAPPER}} .premium-flip-back-content-container' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}}  {{LEFT}}{{UNIT}};',
				),
			)
		);
	...
	protected function render() {
		...
		if ( 'yes' === $settings['premium_flip_icon_fa_switcher'] ) {
			...
			if ( 'icon' === $front_icon || 'svg' === $front_icon ) {
				...
				$this->add_render_attribute( 'front_icon', 'class', array( 'premium-drawable-icon', 'premium-flip-front-icon' ) );
		...
		if ( 'yes' === $settings['premium_flip_back_icon_fa_switcher'] ) {
			...
			if ( 'icon' === $back_icon || 'svg' === $back_icon ) {
				...
				$this->add_render_attribute( 'back_icon', 'class', array( 'premium-drawable-icon', 'premium-flip-back-icon' ) );
		...
		$this->add_render_attribute(
			...
			array(
				'premium-flip-front',
				'premium-flip-front' . $flip_dir,
			)
		);

		if ( 'none' !== $settings['front_lq_effect'] ) {
			$this->add_render_attribute( 'front', 'class', 'premium-con-lq__' . $settings['front_lq_effect'] );
		}
		...
		$this->add_render_attribute(
			...
			array(
				'premium-flip-back',
				'premium-flip-back' . $flip_dir,
			)
		);

		if ( 'none' !== $settings['back_lq_effect'] ) {
			$this->add_render_attribute( 'back', 'class', 'premium-con-lq__' . $settings['back_lq_effect'] );
		}

		?>

	<div <?php echo wp_kses_post( $this->get_render_attribute_string( 'container' ) ); ?>>
		<div <?php echo wp_kses_post( $this->get_render_attribute_string( 'front' ) ); ?>>
			<div class="premium-flip-front-overlay">
				<div class="premium-flip-front-content-container">
					<div class="premium-flip-text-wrapper">
						<?php if ( 'yes' === $settings['premium_flip_icon_fa_switcher'] ) : ?>
							...
							if ( 'icon' === $front_icon ) :
								...
								echo Helper_Functions::get_svg_by_icon(
									...
									$this->get_render_attribute_string( 'front_icon' )
								);
						...
						<?php
						if ( 'yes' === $settings['premium_flip_title_switcher'] && ! empty( $settings['premium_flip_paragraph_header'] ) ) :
		...
		<div <?php echo wp_kses_post( $this->get_render_attribute_string( 'back' ) ); ?>>
					...
					<div class="premium-flip-back-text-wrapper">
						...
						<?php if ( 'yes' === $settings['premium_flip_back_icon_fa_switcher'] ) : ?>
							...
							if ( 'icon' === $back_icon ) :
								...
								echo Helper_Functions::get_svg_by_icon(
									$settings['premium_flip_back_icon_fa_updated'],
									$this->get_render_attribute_string( 'back_icon' )
								);
						...
						<?php
						if ( 'yes' === $settings['premium_flip_back_title_switcher'] && ! empty( $settings['premium_flip_back_paragraph_header'] ) ) :
