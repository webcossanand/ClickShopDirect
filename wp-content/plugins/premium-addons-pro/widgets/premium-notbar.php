...
class Premium_Notbar extends Widget_Base {
    ...
    public function get_script_depends() {
		...
		if ( $is_edit ) {
			...
			if ( 'animation' === $settings['premium_notbar_icon_selector'] ) {
				$scripts[] = 'lottie-js';
			}
		}
	...
	public function get_keywords() {
		return array( 'pa', 'premium', 'premium alert box', 'notification', 'bar', 'popup', 'modal', 'event' );
	}
	...
	protected function register_controls() { // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
        ...
        if ( version_compare( PREMIUM_ADDONS_VERSION, '4.11.24', '>' ) ) {
            $demo = Helper_Functions::get_campaign_link( 'https://premiumaddons.com/elementor-alert-box-widget/', 'alert-box', 'wp-editor', 'demo' );
            Helper_Functions::add_templates_controls( $this, 'alert-box', $demo );
        }
	...
	protected function render() {
		...
		$this->add_render_attribute(
			...
			array(
				...
				'href'       => 'javascript:;',
				'aria-label' => 'Alert Box Close Button',
			)
		);
							...
							<?php if ( 'eicon-ban' !== $settings['close_icon'] ) : ?>
								<div class="premium-notbar-button-wrap">
									<a <?php echo wp_kses_post( $this->get_render_attribute_string( 'close_button' ) ); ?>>
										<i class="<?php echo wp_kses_post( $settings['close_icon'] ); ?>" aria-hidden="true" focusable="false"></i>
									</a>
								</div>
							<?php endif; ?>
						</div>
