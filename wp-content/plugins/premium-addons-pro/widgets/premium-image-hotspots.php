...
class Premium_Image_Hotspots extends Widget_Base {
    ...
    public function get_script_depends() {
		...
		$scripts = array( 'tooltipster-bundle' );

		if ( $is_edit ) {

            $draw_scripts = $this->check_icon_draw() ? array( 'pa-tweenmax', 'pa-motionpath' ) : array();

			$scripts = array_merge( $draw_scripts, array( 'pa-glass', 'lottie-js', 'pa-anime' ) );

		} else {
			$settings = $this->get_settings();

			if ( ! empty( $settings['premium_image_hotspots_icons'] ) ) {

                $draw_js = false;
                $lottie_js = false;

				foreach ( $settings['premium_image_hotspots_icons'] as $item ) {
					...
					if ( ! $lottie_js && 'animation' === $item['premium_image_hotspots_icon_type_switch'] ) {
						$scripts[] = 'lottie-js';
						$lottie_js = true;
					}
            ...
            if ( 'none' !== $settings['tooltip_lq_effect'] ) {
				$scripts[] = 'pa-glass';
			}
	...
	public function get_keywords() {
		return array( 'pa', 'premium', 'premium image hotspots', 'tooltip', 'marker', 'map', 'info', 'box' );
	}
	...
	protected function register_controls() { // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
        ...
        if ( version_compare( PREMIUM_ADDONS_VERSION, '4.11.24', '>' ) ) {
            $demo = Helper_Functions::get_campaign_link( 'https://premiumaddons.com/elementor-image-hotspots-widget/', 'hotspots', 'wp-editor', 'demo' );
            Helper_Functions::add_templates_controls( $this, 'image-hotspots', $demo );
        }
	...
	protected function render() {
		...
		$image_hotspots_settings = array(
			...
			'glass'       => $settings['tooltip_lq_effect'],
		);
