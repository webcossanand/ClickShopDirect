...
class Premium_Tables extends Widget_Base {
	...
	public function get_keywords() {
		return array( 'pa', 'premium', 'premium table', 'sheet', 'data', 'advanced', 'dynamic', 'comparison', 'csv', 'google' );
	}
	...
	protected function register_controls() { // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
		...
		$this->add_control(
			...
			array(
				...
				'type'      => Controls_Manager::SELECT,
				'options'   => array(
					'nocache' => __( 'No Cache', 'premium-addons-pro' ),
		...
		foreach ( $docs as $url => $title ) {
			...
			$doc_url = Helper_Functions::get_campaign_link( $url, 'tables-widget', 'wp-editor', 'get-support' );
			...
			<tr <?php echo wp_kses_post( $this->get_render_attribute_string( 'table_row' ) ); ?>>
				...
				if ( 'custom' === $settings['premium_table_data_type'] ) {
					...
					foreach ( $settings['premium_table_head_repeater'] as $index => $head_cell ) {
						...
						if ( 'none' !== $settings['head_lq_effect'] ) {
							$this->add_render_attribute( 'head-cell-' . $index, 'class', 'premium-con-lq__' . $settings['head_lq_effect'] );
						}
						...
						if ( ! empty( $head_cell['premium_table_cell_icon'] ) || ! empty( $head_cell['premium_table_cell_icon_img']['url'] ) || ! empty( $head_cell['lottie_url'] ) ) {
							...
							$this->add_render_attribute( 'head-cell-icon-' . $index, 'class', 'premium-table-cell-icon-' . $head_cell['premium_table_cell_icon_align'] );
							$this->add_render_attribute( 'head-text-' . $index, 'class', 'premium-table-text' );

							$head .= '<span ' . $this->get_render_attribute_string( 'head-cell-icon-' . $index ) . '>';
							if ( 'font-awesome-icon' === $head_icon_type ) {
								$head .= '<i class="' . esc_attr( $head_cell['premium_table_cell_icon'] ) . '" aria-hidden="true" focusable="false"></i>';
			...
			foreach ( $settings['premium_table_body_repeater'] as $index => $elem ) {
				...
				if ( 'cell' === $elem['premium_table_elem_type'] ) {
					...
					if ( ! empty( $elem['premium_table_cell_icon'] ) || ! empty( $elem['premium_table_cell_icon_img']['url'] ) || ! empty( $elem['lottie_url'] ) ) {
						...
						$this->add_render_attribute( 'body-cell-icon-' . $counter, 'class', 'premium-table-cell-icon-' . $elem['premium_table_cell_icon_align'] );

						$body .= '<span ' . $this->get_render_attribute_string( 'body-cell-icon-' . $counter ) . '>';

						if ( 'font-awesome-icon' === $body_icon_type ) {
							$body .= '<i class="' . esc_attr( $elem['premium_table_cell_icon'] ) . '" aria-hidden="true" focusable="false"></i>';
					...
					$this->add_render_attribute(
						...
						array(
							'premium-table-row',
							'elementor-repeater-item-' . $elem['_id'],
						)
					);

					if ( 'none' !== $settings['rows_lq_effect'] ) {
						$this->add_render_attribute( 'body-row-' . $counter, 'class', 'premium-con-lq__' . $settings['rows_lq_effect'] );
					}
			...
			<thead {{{ view.getRenderAttributeString('table_head') }}}>
				...
				if( 'custom' == settings.premium_table_data_type ) {
					...
					_.each( settings.premium_table_head_repeater, function( headCell, index ) {
						...
						if( 'none' !== settings.head_lq_effect ) {
							view.addRenderAttribute( 'head-cell-' + index, 'class', 'premium-con-lq__' + settings.head_lq_effect );
						}
						...
						<# if( '' != headCell.premium_table_cell_icon || '' != headCell.premium_table_cell_icon_img.url || '' != headCell.lottie_url ){
							...
							view.addRenderAttribute('head-cell-icon-' + index , 'class', 'premium-table-cell-icon-' + headCell.premium_table_cell_icon_align);
							view.addRenderAttribute('head-text-' + index, 'class', 'premium-table-text');

						#>
							<span {{{ view.getRenderAttributeString( 'head-cell-icon-' + index ) }}}>
								<# if( headIconType === 'font-awesome-icon' ){ #>
									<i class="{{ headCell.premium_table_cell_icon }}" aria-hidden="true" focusable="false"></i>
				...
				<#
					...
					_.each( settings.premium_table_body_repeater, function( bodyCell, index ) {
							...
							<{{{bodyCell.premium_table_cell_type}}} {{{ view.getRenderAttributeString('body-cell-' + counter) }}}>
								<{{{htmlTag}}} {{{ view.getRenderAttributeString('body-cell-text-' + counter) }}} >
									...
									<# if( '' != bodyCell.premium_table_cell_icon || '' != bodyCell.premium_table_cell_icon_img.url || '' != bodyCell.lottie_url ) {
										...
										view.addRenderAttribute('body-cell-icon-' + counter , 'class', 'premium-table-cell-icon-' + bodyCell.premium_table_cell_icon_align);

										#>
										<span {{{ view.getRenderAttributeString('body-cell-icon-' + counter) }}}>
											<# if( bodyIconType === 'font-awesome-icon' ){ #>
												<i class="{{ bodyCell.premium_table_cell_icon }}" aria-hidden="true" focusable="false"></i>
						...
						<# } else {
							...
							if( 'none' !== settings.rows_lq_effect ) {
								view.addRenderAttribute( 'body-row-' + counter, 'class', 'premium-con-lq__' + settings.rows_lq_effect );
							}
