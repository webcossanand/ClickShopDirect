...
class Premium_Twitter_Feed extends Widget_Base {
	...
	public function get_keywords() {
		return array( 'pa', 'premium', 'premium twitter feed', 'tweet', 'profile', 'account', 'tag', 'social' );
	}
	...
	protected function register_controls() { // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
		...
		$this->start_controls_section(
			'credentials_section',
			array(
				'label' => __( 'Credentials', 'premium-addons-pro' ),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'label_block' => true,
                'description' => __('Add your account username without @. For example, leapworx')
			)
		);
		...
		$this->add_control(
			'bearer_token',
			array(
				'label'       => __( 'Bearer Token', 'premium-addons-pro' ),
				'type'        => Controls_Manager::TEXT,
				'label_block' => true,
	...
	protected function render() {
		...
		if ( empty( $settings['username'] ) || empty( $settings['bearer_token'] ) ) {

			?>
			<div class="premium-error-notice">
				<?php echo esc_html( __( 'Please fill the required fields: Bearer Token and Username', 'premium-addons-pro' ) ); ?>
			</div>
			<?php
			return;
		}
		...
		$transient_name = sprintf( 'papro_tweets_%s_%s', $username, $id );
		...
		if ( false === $data ) {
			...
			$user_api = sprintf( 'https://api.twitter.com/2/users/by/username/%s?user.fields=profile_image_url,profile_banner_url,public_metrics', $username );
			...
			if ( is_wp_error( $user_info_response ) || 200 !== wp_remote_retrieve_response_code( $user_info_response ) ) {
				?>
			<div class="premium-error-notice">
				<?php echo esc_html( __( 'Something went wrong. Please try again in few minutes', 'premium-addons-pro' ) ); ?>
			</div>
				<?php
				return;
			}
			...
			$tweets_api = sprintf( 'https://api.twitter.com/2/users/%s/tweets?max_results=100&tweet.fields=entities,public_metrics,in_reply_to_user_id,attachments,created_at', $user_info->data->id );
            ...
            if ( is_wp_error( $tweets_response ) || 200 !== wp_remote_retrieve_response_code( $tweets_response ) ) {
				?>
			<div class="premium-error-notice">
				<?php echo esc_html( __( 'Something went wrong. Please try again in few minutes', 'premium-addons-pro' ) ); ?>
			</div>
			...
			set_transient( $transient_name, $data, $expire_time );
