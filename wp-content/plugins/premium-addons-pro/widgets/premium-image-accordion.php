...
class Premium_Image_Accordion extends Widget_Base {
    ...
    public function get_script_depends() {
		...
		if ( $is_edit ) {
			...
			if ( ! empty( $settings['image_content'] ) ) {

                $draw_js = false;
                $lottie_js = false;

				foreach ( $settings['image_content'] as $item ) {
            ...
            if ( 'none' !== $settings['overlay_lq_effect'] ) {
				$scripts[] = 'pa-glass';
			}
		}
	...
	public function get_keywords() {
		return array( 'pa', 'premium', 'premium image accordion', 'image', 'photo', 'visual', 'slide' );
	}
	...
	protected function register_controls() { // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
		...
		$doc_url = Helper_Functions::get_campaign_link( 'https://premiumaddons.com/docs/image-accordion-widget-tutorial/', 'accordion-widget', 'wp-editor', 'get-support' );
		...
		$this->add_control(
			...
			array(
				...
				'options'     => array(
					...
					'glass1-hover' => __( 'Preset 01', 'premium-addons-for-elementor' ),
					'glass2-hover' => __( 'Preset 02', 'premium-addons-for-elementor' ),
	...
	protected function render() {
		...
		$this->add_render_attribute( 'overlay', 'class', 'premium-accordion-overlay-wrap' );

		if ( 'none' !== $settings['overlay_lq_effect'] ) {
			$this->add_render_attribute( 'overlay', 'class', 'premium-con-lq__' . $settings['overlay_lq_effect'] );
		}
					...
					foreach ( $settings['image_content'] as $index => $item ) :
						...
						if ( 'yes' === $item['link_switcher'] ) {
							...
							if ( ! empty( $item['link']['url'] ) || ! empty( $item['existing_link'] ) ) {
								$this->add_render_attribute( $item_link, 'href', esc_url( $link_url ) );
							}
						}
						...
						<li <?php echo wp_kses_post( $this->get_render_attribute_string( $list_item_key ) ); ?>>
							...
							<div <?php echo wp_kses_post( $this->get_render_attribute_string( 'overlay' ) ); ?>>
