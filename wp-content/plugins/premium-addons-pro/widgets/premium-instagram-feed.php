...
class Premium_Instagram_Feed extends Widget_Base {
    ...
    public function get_script_depends() {
		...
		$scripts = array( 'imagesloaded', 'pa-instafeed' );

		if ( $is_edit ) {

			$scripts = array_merge( $scripts, array( 'pa-glass', 'prettyPhoto-js', 'pa-slick', 'isotope-js' ) );
			...
			if ( 'yes' === $settings['premium_instagram_feed_masonry']) {
				$scripts[] = 'isotope-js';
			}
            ...
            if ( 'none' !== $settings['share_lq_effect'] ) {
				$scripts[] = 'pa-glass';
			}
		}
	...
	public function get_keywords() {
		return array( 'pa', 'premium', 'premium instagram feed', 'profile', 'account', 'post', 'social' );
	}
	...
	protected function register_controls() { // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
		...
		$this->add_control(
			...
			array(
				...
				'raw'         => '<form onsubmit="connectInstagramInit(this);" action="javascript:void(0);" data-type="reviews"><input type="submit" value="Log in with Instagram" class="elementor-button" style="background: linear-gradient(45deg, #f09433 0%, #e6683c 25%, #dc2743 50%, #cc2366 75%, #bc1888 100%); color: #fff;"></form>',
				'label_block' => true,
				'condition'   => array(
					'api_version' => 'new',
				),
		...
		$this->add_control(
			'show_videos',
			array(
				'label'     => __( 'Videos Behavior', 'premium-addons-pro' ),
				'type'      => Controls_Manager::SELECT,
				'options'   => array(
                    'none'  => __( 'None', 'premium-addons-pro' ),
					'redirect'  => __( 'Redirect', 'premium-addons-pro' ),
					'hover' => __( 'Play On Hover', 'premium-addons-pro' ),
					'click' => __( 'Play On Click', 'premium-addons-pro' ),
				),
				'default'   => 'none',
				'condition' => array(
					'premium_instagram_feed_popup!' => 'yes',
		...
		foreach ( $docs as $url => $title ) {
			...
			$doc_url = Helper_Functions::get_campaign_link( $url, 'instagram-widget', 'wp-editor', 'get-support' );
		...
		$this->add_responsive_control(
			...
			array(
				...
				'size_units' => array( 'px', 'em', '%' ),
				'selectors'  => array(
					'{{WRAPPER}} .premium-insta-image-caption' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				),
			)
		);
		...
		$this->start_controls_section(
			'play_icon_style',
			array(
				'label'     => __( 'Play Icon', 'premium-addons-pro' ),
				'tab'       => Controls_Manager::TAB_STYLE,
				'condition' => array(
					'show_videos!' => ['redirect', 'none'],
				),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'type'      => Controls_Manager::COLOR,
				'selectors' => array(
					'{{WRAPPER}} .premium-insta-feed-play-icon' => 'color: {{VALUE}};',
				),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'type'      => Controls_Manager::COLOR,
				'selectors' => array(
					'{{WRAPPER}} .premium-insta-feed-play-icon' => 'background-color: {{VALUE}};',
				),
			)
		);
		...
		$this->add_group_control(
			...
			array(
				...
				'selector' => '{{WRAPPER}} .premium-insta-feed-play-icon',
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'size_units' => array( 'px', '%', 'em' ),
				'selectors'  => array(
					'{{WRAPPER}} .premium-insta-feed-play-icon' => 'border-radius: {{SIZE}}{{UNIT}};',
				),
			)
		);
		...
		$this->add_responsive_control(
			...
			array(
				...
				'size_units' => array( 'px', 'em', '%' ),
				'selectors'  => array(
					'{{WRAPPER}} .premium-insta-feed-play-icon' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				),
			)
		);
	...
	protected function render() {
		...
		if ( $share ) {
			...
			$share_menu_class = 'premium-instafeed-share-menu premium-con-lq__' . $settings['share_lq_effect'];
            ...
            <span class="premium-instafeed-share-button">
                ...
                <div class="' . $share_menu_class . '">
                    ...
                    onclick="window.open(\'https://www.facebook.com/sharer/sharer.php?u={{link}}\',\'popup\',\'width=600,height=600\'); return false;">
                        <i class="fab fa-facebook-f if-fb" aria-hidden="true" focusable="false"></i>
                    ...
                    onclick="window.open(\'https://twitter.com/intent/tweet?text=tweet&url={{link}}\',\'popup\',\'width=600,height=600\'); return false;">
                        <i class="fab fa-twitter if-tw" aria-hidden="true" focusable="false"></i>
                    ...
                    onclick="window.open(\'https://www.pinterest.com/pin/create/button/?url={{link}}\',\'popup\',\'width=600,height=600\'); return false;">
                        <i class="fab fa-pinterest-p if-pi" aria-hidden="true" focusable="false"></i>
		...
		} elseif ( 'yes' === $settings['premium_instagram_feed_popup'] ) {
		...
		$instagram_settings = array(
			...
			'share'       => $share_html,
			'overlay'     => isset( $settings['premium_instgram_feed_overlay_background'] ) ? $settings['premium_instgram_feed_overlay_background'] : false,
			'feed'        => $response,
