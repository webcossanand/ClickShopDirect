...
class Premium_Google_Reviews extends Widget_Base {
	...
	public function get_keywords() {
		return array( 'pa', 'premium', 'premium google reviews', 'business', 'rating', 'testimonials', 'place', 'rate', 'recommendation', 'social' );
	}
	...
	public function get_script_depends() {
		...
		if ( $is_edit ) {
			...
			$scripts = array( 'pa-slick', 'isotope-js' );

		} else {
			$settings = $this->get_settings();

			if ( 'masonry' === $settings['reviews_style']) {
				$scripts[] = 'isotope-js';
			}

			if ( 'yes' === $settings['reviews_carousel'] ) {
				$scripts[] = 'pa-slick';
			}
		}
	...
	protected function register_controls() { // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
		...
		$this->add_control(
			...
			array(
				...
				'options'     => array(
					'old' => __( 'Place Details (Old)', 'premium-addons-pro' ),
					'new' => __( 'Places (New)', 'premium-addons-pro' ),
				),
				'default'     => 'old',
				'label_block' => true,
			)
		...
		$this->add_control(
			...
			array(
				...
				'description' => __( 'This helps to control number of reviews string. {{number}} will be replaced with the number of reviews', 'premium-addons-pro' ),
				'condition'   => array(
					'place_info'     => 'yes',
					'reviews_number' => 'yes',
				),
		...
		$this->add_control(
			'chars_num',
			array(
				'label'     => __( 'Review Characters Length', 'premium-addons-pro' ),
				'type'      => Controls_Manager::NUMBER,
				'min'       => 1,
				'condition' => array(
					'text'            => 'yes',
		...
		$docs = array(
			...
			'https://premiumaddons.com/docs/google-api-key-for-elementor-widgets/' => __( 'How to get an API key for Google Reviews »', 'premium-addons-pro' ),
		...
		foreach ( $docs as $url => $title ) {
			...
			$doc_url = Helper_Functions::get_campaign_link( $url, 'google-reviews-widget', 'wp-editor', 'get-support' );
	...
	protected function render() {
		...
		if ( false === $response ) {
			...
			$response = premium_google_rev_api_rating( $api_key, $version, $place_id, $lang_prefix, $sort );
		...
		if ( ! $response_results || ! property_exists( $response_results, 'reviews' ) ) {
			?>
			<div class="premium-error-notice">
				<?php
					echo wp_kses_post( sprintf( 'Something went wrong, error' ) );
				?>
			</div>
			<?php
		...
		$chars_length = $settings['chars_num'];
		...
		if ( 'yes' === $settings['place_info'] && 'yes' === $settings['reviews_number'] ) {
			$place_settings['number_text'] = str_replace( '{{number}}', '%s', $settings['reviews_number_text'] );
		}
		...
		if ( 'yes' === $settings['schema'] ) {
			...
			if ( 'old' === $version ) {
				...
				$ratings = $response_results->userRatingCount;

				$address = $response_results->formattedAddress;

				$name = $response_results->displayName->text;
			...
			<div class="premium-fb-rev-list">
				<?php if ( 'yes' === $settings['place_info'] ) : ?>
					<div <?php echo wp_kses_post( $this->get_render_attribute_string( 'place' ) ); ?>>
						<div class="premium-fb-rev-page-inner">
							<?php
							if ( 'old' === $version ) :
								premium_reviews_place( $place, $place_settings );
							else :
								premium_reviews_place_new( $place, $place_settings );
							endif;
							?>
				...
				<?php if ( ! empty( $reviews ) ) : ?>
					<div <?php echo wp_kses_post( $this->get_render_attribute_string( 'reviews' ) ); ?>>
						<?php
						if ( 'old' === $version ) :
							premium_google_rev_reviews( $reviews, $reviews_settings );
						else :
							premium_google_rev_reviews_new( $reviews, $reviews_settings );
						endif;
						?>
					</div>
				<?php endif; ?>
